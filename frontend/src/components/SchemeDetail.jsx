    ...
    if (ep.caste?.length) {
      ...
      if (userCaste && !ep.caste.some((c) => c.toLowerCase() === userCaste)) {
        ...
        const allowedRank = Math.min(
          ...ep.caste.map((c) => casteRank[c.toLowerCase()] || 5),
        );
        ...
        {!result ? (
          ...
          <>
            <div
              ...
              style={{
                backgroundColor: result.eligible ? "#10a37f20" : "#37415120",
                color: result.eligible ? "#10a37f" : "var(--text-primary)",
              }}
            >
              {result.verdict}
            </div>
...
export function SchemeDetail({ scheme, onBack }) {
  ...
  const handleSend = async (clearHistory = false) => {
    ...
    try {
      ...
      const response = await api.schemeChat(
        safeScheme,
        text,
        history,
        clearHistory || firstMessage,
      );
  ...
  return (
    ...
    >
      <header
        className="sticky top-0 z-20 px-4 h-[60px] flex items-center gap-3 border-b"
        style={{
      ...
      <ChatInput
        ...
        onSend={handleSend}
        disabled={loading}
        placeholder="Is scheme ke baare mein poochhein..."
      />
