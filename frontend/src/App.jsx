...
const initialGreeting = (
  <div style={{ lineHeight: 1.6 }}>
    <strong style={{ fontSize: '1.1rem' }}>Namaste! 🙏</strong><br /><br />
    ...
    <span style={{ color: 'var(--text-muted)', fontSize: '0.9rem' }}>Jaise: padhai ke liye loan, kisan subsidy, business, naukri...</span>
  </div>
);
...
function App() {
  ...
  const addBotMessage = (content) => setMessages(prev => [...prev, { type: 'bot', content }]);

  const showQuestion = (index) => {
    if (index < profileQuestions.length) {
      const q = profileQuestions[index];
      let hint = q.hint || '';
      if (q.options) hint = Object.entries(q.options).map(([k, v]) => `${k} = ${v}`).join('  ');
      const fullText = `${q.label} ${hint}`;
      speak(fullText);
      addBotMessage(<div style={{ color: 'var(--text-primary)' }}>{q.label}{hint && <div style={{ color: 'var(--text-muted)', fontSize: '0.85rem', marginTop: '4px' }}>{hint}</div>}</div>);
    }
  };
  ...
  const sendProfileAnswers = async () => {
    ...
    } catch (err) { addBotMessage(`Error: ${err.message}`); }
    setLoading(false);
  };
  ...
  if (selectedScheme) return <SchemeDetail scheme={selectedScheme} onBack={closeSchemeDetail} />;
    ...
    <div className="min-h-screen flex flex-col" style={{ backgroundColor: 'var(--bg-primary)' }}>
      ...
      <div className="flex-1 max-w-3xl w-full mx-auto flex flex-col h-[calc(100vh-60px)]">
        <div className="flex-1 overflow-y-auto px-4 sm:px-6 py-4 flex flex-col gap-4">
          {messages.map((msg, i) => (<Message key={i} type={msg.type}>{typeof msg.content === 'string' ? <Bubble type={msg.type}>{msg.content}</Bubble> : msg.content}</Message>))}
